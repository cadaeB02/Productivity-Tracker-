...
export async function uploadDocument({ file, company_id, category, description, linked_type, linked_id }) {
    const { data: { user } } = await supabase.auth.getUser();
...
export async function addTransaction({ type, amount, description, category, company_id, date, is_recurring }) {
    const { data: { user } } = await supabase.auth.getUser();
    const insertData = {
        user_id: user.id,
        company_id,
